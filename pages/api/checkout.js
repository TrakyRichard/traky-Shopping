...
export default async (req, res) => {
  ...
  try {
    ...
    // 2) Find Card based on UserId, populate it
    ...
    //4) Get Email from payement data, see if email linked with existing Stripe Customer
    const prevCustomer = await stripe.customers.list({
      email: paymentData.email,
      limit: 1
    });
    const isExistingCustomer = prevCustomer.data.length > 0;
    // 5) if not existing custumer, create them based on their email
    let newCustomer;
